...
> **Engineering Deep Dive: 동적 패딩(Dynamic Padding)을 통한 병목 최적화**
> 
> 일반적인 기초 모델링 튜토리얼에서는 텐서 크기를 맞추기 위해 `max_length=128` 등으로 길이를 하드코딩합니다. 하지만 프로덕션 환경에서 이는 두 가지 치명적인 낭비를 발생시킵니다.
> 
> 1. **네트워크 대역폭(I/O) 낭비:** "Hello"라는 짧은 문장(토큰 2개)을 서버에 요청할 때도 128개의 배열을 전송해야 하므로 네트워크 페이로드가 불필요하게 커집니다.
> 2. **GPU 연산량(Compute) 낭비:** 트랜스포머의 셀프 어텐션(Self-Attention) 메커니즘은 $O(N^2)$의 시간 복잡도를 가집니다. 시퀀스 길이($N$)가 고정되어 있으면, 불필요한 패딩 토큰까지 모두 Attention 연산을 수행해야 합니다.
> 
> 본 파이프라인은 클라이언트 단에서 `padding='longest'`를 적용해 **입력된 문장 길이에 딱 맞게 텐서를 타이트하게 생성**하고, Triton 서버의 **Dynamic Axes** 옵션과 결합했습니다. 이를 통해 네트워크 트래픽을 최소화하고, 짧은 문장 처리 시 GPU 연산 속도를 기하급수적으로 끌어올렸습니다.
